...
if(randomVariable=="task"){
$(document).ready(function() {
	
	  $.ajax({
	    url: '/getTaskIdDeatails',
	    type: 'post',
...
// Add an "Action" column header
	  ...
	  //costomize the order for required fields
	    /* const columnsOrder = ["taskId", "taskName", "estStartDate" ,"estEndDate", "taskStatus", "employeeId", "assignName", "findingNo", "masterMeetingId", "description", "remarks", "transferTo"];
	    ...
	    //costomize the order for required fields
		  const columnsOrder = [
		                      ...
		                      columnsOrder.forEach(columnName => {
		                        if (data[0].hasOwnProperty(columnName)) {
		                          const th = document.createElement("th");
		                          th.textContent = columnName.replace(/([A-Z])/g, ' $1').replace(/^./, function(str) {
   ...
   /*  for (const key in data[0]) {
    ...
    console.log("hello sag:"+headerRow);
    /*
	// Add an "Action" column header
	  const actionHeader = document.createElement("th");
	  actionHeader.textContent = "Action";
    ...
    for (let i = 0; i < data.length; i++) {
	 ...
	 // Customize the order of other columns as needed
        columnsOrder.forEach(columnName => {
            if (data[i].hasOwnProperty(columnName)) {
                const cell = row.insertCell();
                cell.textContent = data[i][columnName];
            }
        });
...
function updateTaskDetails (element){
 	...
 	let TaskObj = JSON.parse(element.value);
 	console.log(TaskObj);
 	  if (TaskObj.employeeId == userEmpId || userType=='admin') {
